...
public class Juego {
    ...
    class capaJuego extends Layer {
        ...
        void ponerPlataforma() {
            Sprite _Plataforma;
            Log.d("PonerPlataforma", "Voy a armar el Sprite de la plataforma");
            _Plataforma = Sprite.sprite("Plataforma.png");

            Log.d("PonerPlataforma", "Determino posicion inical");
            ...
            posicionInicialPlataforma.y = _Pantalla.getHeight();

            Log.d("PonerPlataforma", "Ubico el Sprite");
            _Plataforma.setPosition(posicionInicialPlataforma.x, posicionInicialPlataforma.y);

            Log.d("PonerPlataforma", "Lo agrego a la capa");
            super.addChild(_Plataforma, 7);

            Log.d("PonerPlataforma", "Lo agrego al Array");
            _listaPlataforma.add(_Plataforma);
            Log.d("PonerPlataforma", "Ya tengo " + _listaPlataforma.size() + " enemigos");
...
//Me fijo si el vértice superior derecho de Sp2 está dentro de Sp1
        ...
        public void detectarColisiones(float deltaTiempo) {
            Log.d("DetectarColisiones", "Me fijo si alguna plataforma colisionó con el jugador");

            boolean huboColision;
            huboColision = false;

            ArrayList plataformasQueImpactaron;
            plataformasQueImpactaron=new ArrayList();

            for (int punteroPlataforma = 0; punteroPlataforma < _listaPlataforma.size(); punteroPlataforma++) {
                Log.d("DetectarColisiones", "Verifico el enemigo número " + punteroPlataforma);

                Sprite unaPlataformaAVerificar;
                unaPlataformaAVerificar = (Sprite) _listaPlataforma.get(punteroPlataforma);

                if (IntereseccionEntreSprites(_Avatar, unaPlataformaAVerificar)) ;
                {
                    huboColision=true;
                    super.removeChild(unaPlataformaAVerificar, true);
                    plataformasQueImpactaron.add(punteroPlataforma);
                }
            }

            if(huboColision == true){
                Log.d("DetectarColisiones", "MATEO GOL!!!!!!!");

                Log.d("DetectarColisiones", "Remuevo las plataformas que impactaron al Array");
                for (int punteroPlataforma=plataformasQueImpactaron.size()-1;punteroPlataforma>=0; punteroPlataforma--){
                    _listaPlataforma.remove(punteroPlataforma);
                }
                Log.d("DetectarColisiones", "Me quedan "+_listaPlataforma.size()+" plataformas");
            }
        }
    }
}
